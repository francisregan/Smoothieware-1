...
#define shoulder_arm_length_checksum          CHECKSUM("shoulder_arm_length")
#define elbow_arm_length_checksum          CHECKSUM("elbow_arm_length")
#define shoulder_master_gear_teeth_count_checksum  CHECKSUM("shoulder_master_gear_teeth_count")
#define shoulder_slave_gear_teeth_count_checksum		CHECKSUM("shoulder_slave_gear_teeth_count")
#define elbow_master_gear_teeth_count_checksum		CHECKSUM("elbow_master_gear_teeth_count")
#define elbow_slave_gear_teeth_count_checksum		CHECKSUM("elbow_slave_gear_teeth_count")

#define SQ(x) powf(x, 2)
#define ROUND(x, y) (roundf(x * 1e ## y) / 1e ## y)

GearScaraSolution::GearScaraSolution(Config* config)
{
    // arm1_length is the length of the inner main arm from hinge to hinge
    arm1_length = config->value(shoulder_arm_length_checksum)
    									->by_default(150.0f)->as_number();
    // arm2_length is the length of the inner main arm from hinge to hinge
    arm2_length = config->value(elbow_arm_length_checksum)
    									->by_default(150.0f)->as_number();
    shoulder_master_gear_teeth_count = config->value(shoulder_master_gear_teeth_count_checksum)
    									->by_default(12)->as_int();
    shoulder_slave_gear_teeth_count = config->value(shoulder_slave_gear_teeth_count_checksum)
    									->by_default(108)->as_int();
    elbow_master_gear_teeth_count = config->value(elbow_master_gear_teeth_count_checksum)
    									->by_default(12)->as_int();
    elbow_slave_gear_teeth_count= config->value(elbow_slave_gear_teeth_count_checksum)
    									->by_default(68)->as_int();

    init();
}

void GearScaraSolution::init() {

}

float GearScaraSolution::to_degrees(float radians) {
    return radians*(180.0F/3.14159265359f);
}

void GearScaraSolution::cartesian_to_actuator(const float cartesian_mm[], ActuatorCoordinates &actuator_mm )
{
...
void GearScaraSolution::actuator_to_cartesian(const ActuatorCoordinates &actuator_mm, float cartesian_mm[] ) {
...
bool GearScaraSolution::set_optional(const arm_options_t& options) {
/*
...
bool GearScaraSolution::get_optional(arm_options_t& options, bool force_all) {
	/*
